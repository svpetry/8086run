...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/debug.ts");
/******/ })
/************************************************************************/
/******/ ({
...
class DebugHelper {
    constructor() {
        this.instructionLines = new Array();
        this.lastAddr = 0;
        this.regAx = document.getElementById("AX");
        this.regBx = document.getElementById("BX");
        this.regCx = document.getElementById("CX");
        this.regDx = document.getElementById("DX");
        this.regSp = document.getElementById("SP");
        this.regBp = document.getElementById("BP");
        this.regSi = document.getElementById("SI");
        this.regDi = document.getElementById("DI");
        this.regCs = document.getElementById("CS");
        this.regDs = document.getElementById("DS");
        this.regEs = document.getElementById("ES");
        this.regSs = document.getElementById("SS");
        this.regIp = document.getElementById("IP");
        this.regFlags = document.getElementById("FLAGS");
        this.instructionHistory = document.getElementById("instruction-history");
        this.maxLines = 100;
    }
    showRegisters(emulator) {
        this.regAx.innerHTML = emulator.getAX().toString(16).padStart(4, "0").toUpperCase();
        this.regBx.innerHTML = emulator.getBX().toString(16).padStart(4, "0").toUpperCase();
        this.regCx.innerHTML = emulator.getCX().toString(16).padStart(4, "0").toUpperCase();
        this.regDx.innerHTML = emulator.getDX().toString(16).padStart(4, "0").toUpperCase();
        this.regSp.innerHTML = emulator.getSP().toString(16).padStart(4, "0").toUpperCase();
        this.regBp.innerHTML = emulator.getBP().toString(16).padStart(4, "0").toUpperCase();
        this.regSi.innerHTML = emulator.getSI().toString(16).padStart(4, "0").toUpperCase();
        this.regDi.innerHTML = emulator.getDI().toString(16).padStart(4, "0").toUpperCase();
        this.regCs.innerHTML = emulator.getCS().toString(16).padStart(4, "0").toUpperCase();
        this.regDs.innerHTML = emulator.getDS().toString(16).padStart(4, "0").toUpperCase();
        this.regEs.innerHTML = emulator.getES().toString(16).padStart(4, "0").toUpperCase();
        this.regSs.innerHTML = emulator.getSS().toString(16).padStart(4, "0").toUpperCase();
        this.regIp.innerHTML = emulator.getIP().toString(16).padStart(4, "0").toUpperCase();
        this.regFlags.innerHTML = emulator.getFLAGS().toString(2).padStart(16, "0").toUpperCase();
    }
    disassembleSingle(emulator) {
        emulator.disassembler.doDisassemble();
        const currentInstruction = emulator.disassembler.disassembledLine;
        const pre = document.createElement("pre");
        pre.innerHTML = currentInstruction;
        this.instructionHistory.appendChild(pre);
        if (this.instructionHistory.childNodes.length >= this.maxLines) {
            this.instructionHistory.removeChild(this.instructionHistory.childNodes[0]);
        }
        this.instructionHistory.scrollTop = this.instructionHistory.scrollHeight;
    }
    clearInstructionHistory() {
        this.instructionHistory.innerHTML = "";
    }
    clearInstructionBuffer() {
        this.instructionLines.length = 0;
    }
    showInstructionBuffer() {
        this.instructionHistory.innerHTML = "";
        for (let idx = this.instructionLines.length - this.maxLines; idx < this.instructionLines.length; idx++) {
            const pre = document.createElement("pre");
            pre.innerHTML = this.instructionLines[idx];
            this.instructionHistory.appendChild(pre);
        }
        this.instructionHistory.scrollTop = this.instructionHistory.scrollHeight;
    }
    addInstructionLineToBuffer(emulator) {
        emulator.disassembler.doDisassemble();
        const addr = (emulator.getCS() << 4) + emulator.getIP();
        if (this.lastAddr != addr) {
            this.lastAddr = addr;
            this.instructionLines.push(emulator.disassembler.disassembledLine);
        }
        if (this.instructionLines.length > this.maxLines)
            this.instructionLines.shift();
    }
    downloadHistory() {
        const history = this.instructionLines.join("\r\n");
        utils_1.Utils.download("instructions.txt", history);
    }
}
...
const debug_helper_1 = __webpack_require__(/*! ./debug-helper */ "./src/debug-helper.ts");
const utils_1 = __webpack_require__(/*! ./utils */ "./src/utils.ts");
...
class Debug {
    constructor() {
        ...
        settings_1.Settings.cpuType = cpuUtils_1.CpuType.Cpu186;
        settings_1.Settings.ramAmount = 1024;
        settings_1.Settings.enableRtc = true;
        settings_1.Settings.enableUma = true;
        settings_1.Settings.hdImageDir = "HDImages/";
        settings_1.Settings.graphicsAdapterType = settings_1.GraphicsType.CGA;
        this.debugHelper = new debug_helper_1.DebugHelper();
        this.emulator = new emulator_1.Emulator(10);
        ...
        document.addEventListener("keydown", (ev) => {
            if (ev.repeat)
                return;
            this.emulator.sendKey(utils_1.Utils.getScanCode(ev.code, utils_1.KeyAction.Press));
            ev.preventDefault();
            ev.stopPropagation();
            return false;
        });
        document.addEventListener("keyup", (ev) => {
            this.emulator.sendKey(utils_1.Utils.getScanCode(ev.code, utils_1.KeyAction.Release));
            ev.preventDefault();
            ev.stopPropagation();
            return false;
        });
    ...
    doRun() {
        for (let i = 0; i < 2500 && this.running; i++) {
            this.emulator.step();
            this.debugHelper.addInstructionLineToBuffer(this.emulator);
        }
        this.runStep++;
        if (this.runStep == 20) {
            this.runStep = 0;
            this.debugHelper.showRegisters(this.emulator);
        }
        if (this.running) {
            setTimeout(() => {
                this.doRun();
            ...
            this.debugHelper.showInstructionBuffer();
    }
    async doReset() {
        this.debugHelper.clearInstructionHistory();
        this.debugHelper.clearInstructionBuffer();
        await this.emulator.reset();
        this.emulator.step();
        this.debugHelper.disassembleSingle(this.emulator);
        this.debugHelper.showRegisters(this.emulator);
    }
    async reset() {
        this.running = false;
        setTimeout(this.doReset, 20);
    }
    step() {
        if (this.running)
            return;
    ...
    async startEmulation() {
        await this.emulator.loadRom("Roms/BIOS/PCXTBIOS.BIN", 0xFE000);
        await this.emulator.loadRom("Roms/Harddisk/IBM_XEBEC_62X0822_1985.BIN", 0xC8000);
        await this.emulator.prepare();
        this.debugHelper.disassembleSingle(this.emulator);
        const screenHelper = new screen_1.ScreenHelperSync(this.emulator);
        window.requestAnimationFrame(() => screenHelper.animationFrame());
    }
}
new Debug();
